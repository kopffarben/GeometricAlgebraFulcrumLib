using System;
using System.Diagnostics;
using GeometricAlgebraFulcrumLib.Algebra.Scalars.Float64;
using GeometricAlgebraFulcrumLib.Algebra.Scalars.Generic;
using GeometricAlgebraFulcrumLib.Modeling.Trajectories.Scalars.Float64.Normalized;
using GeometricAlgebraFulcrumLib.Modeling.Trajectories.Scalars.Generic.Normalized;
using NUnit.Framework;

namespace GeometricAlgebraFulcrumLib.UnitTests.Modeling.Signals;

[TestFixture]
public sealed class ScalarTriangleSignalEquivalenceTests
{
    private const double Tolerance = 1e-12;
    private static readonly ScalarProcessorOfFloat64 ScalarProcessor = ScalarProcessorOfFloat64.Instance;

    [Test]
    public void TestFiniteSymmetricTriangleSignal_BoundaryValues()
    {
        var float64Signal = Float64ScalarTriangleSignal.FiniteSymmetric;
        var genericSignal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);

        // Test at t = -1 (start)
        var t1 = -1.0;
        var float64Value1 = float64Signal.GetValue(t1);
        var genericValue1 = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t1)).ScalarValue;
        Assert.That(Math.Abs(float64Value1 - genericValue1), Is.LessThan(Tolerance),
            $"Mismatch at t={t1}: Float64={float64Value1}, Generic={genericValue1}");
        Assert.That(float64Value1, Is.EqualTo(-1.0).Within(Tolerance));

        // Test at t = 0 (vertex/peak)
        var t2 = 0.0;
        var float64Value2 = float64Signal.GetValue(t2);
        var genericValue2 = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t2)).ScalarValue;
        Assert.That(Math.Abs(float64Value2 - genericValue2), Is.LessThan(Tolerance),
            $"Mismatch at t={t2}: Float64={float64Value2}, Generic={genericValue2}");
        Assert.That(float64Value2, Is.EqualTo(1.0).Within(Tolerance));

        // Test at t = 1 (end)
        var t3 = 1.0;
        var float64Value3 = float64Signal.GetValue(t3);
        var genericValue3 = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t3)).ScalarValue;
        Assert.That(Math.Abs(float64Value3 - genericValue3), Is.LessThan(Tolerance),
            $"Mismatch at t={t3}: Float64={float64Value3}, Generic={genericValue3}");
        Assert.That(float64Value3, Is.EqualTo(-1.0).Within(Tolerance));

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteSymmetricTriangleSignal_RisingSlope()
    {
        var float64Signal = Float64ScalarTriangleSignal.FiniteSymmetric;
        var genericSignal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);

        // Test rising slope from -1 to 0
        for (double t = -1.0; t <= 0.0; t += 0.1)
        {
            var float64Value = float64Signal.GetValue(t);
            var genericValue = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t)).ScalarValue;
            Assert.That(Math.Abs(float64Value - genericValue), Is.LessThan(Tolerance),
                $"Mismatch at t={t}: Float64={float64Value}, Generic={genericValue}");
        }

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteSymmetricTriangleSignal_FallingSlope()
    {
        var float64Signal = Float64ScalarTriangleSignal.FiniteSymmetric;
        var genericSignal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);

        // Test falling slope from 0 to 1
        for (double t = 0.0; t <= 1.0; t += 0.1)
        {
            var float64Value = float64Signal.GetValue(t);
            var genericValue = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t)).ScalarValue;
            Assert.That(Math.Abs(float64Value - genericValue), Is.LessThan(Tolerance),
                $"Mismatch at t={t}: Float64={float64Value}, Generic={genericValue}");
        }

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteAsymmetricTriangleSignal_LeftVertex()
    {
        // Vertex at t=-0.5 (vertexRelativeTime=0.25)
        var vertexRelativeTime = 0.25;
        var vertexTime = 2 * vertexRelativeTime - 1; // -0.5

        var float64Signal = Float64ScalarTriangleSignal.Finite(vertexRelativeTime);
        var genericSignal = ScalarTriangleSignal<double>.Finite(ScalarProcessor, ScalarProcessor.ScalarFromNumber(vertexTime));

        // Test at vertex
        var float64Value = float64Signal.GetValue(vertexTime);
        var genericValue = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(vertexTime)).ScalarValue;
        Assert.That(Math.Abs(float64Value - genericValue), Is.LessThan(Tolerance),
            $"Mismatch at vertex t={vertexTime}: Float64={float64Value}, Generic={genericValue}");
        Assert.That(float64Value, Is.EqualTo(1.0).Within(Tolerance));

        // Test full range
        for (double t = -1.0; t <= 1.0; t += 0.2)
        {
            float64Value = float64Signal.GetValue(t);
            genericValue = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t)).ScalarValue;
            Assert.That(Math.Abs(float64Value - genericValue), Is.LessThan(Tolerance),
                $"Mismatch at t={t}: Float64={float64Value}, Generic={genericValue}");
        }

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteAsymmetricTriangleSignal_RightVertex()
    {
        // Vertex at t=0.5 (vertexRelativeTime=0.75)
        var vertexRelativeTime = 0.75;
        var vertexTime = 2 * vertexRelativeTime - 1; // 0.5

        var float64Signal = Float64ScalarTriangleSignal.Finite(vertexRelativeTime);
        var genericSignal = ScalarTriangleSignal<double>.Finite(ScalarProcessor, ScalarProcessor.ScalarFromNumber(vertexTime));

        // Test at vertex
        var float64Value = float64Signal.GetValue(vertexTime);
        var genericValue = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(vertexTime)).ScalarValue;
        Assert.That(Math.Abs(float64Value - genericValue), Is.LessThan(Tolerance),
            $"Mismatch at vertex t={vertexTime}: Float64={float64Value}, Generic={genericValue}");
        Assert.That(float64Value, Is.EqualTo(1.0).Within(Tolerance));

        // Test full range
        for (double t = -1.0; t <= 1.0; t += 0.2)
        {
            float64Value = float64Signal.GetValue(t);
            genericValue = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t)).ScalarValue;
            Assert.That(Math.Abs(float64Value - genericValue), Is.LessThan(Tolerance),
                $"Mismatch at t={t}: Float64={float64Value}, Generic={genericValue}");
        }

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteSymmetricTriangleSignal_Derivative1()
    {
        var float64Signal = Float64ScalarTriangleSignal.FiniteSymmetric;
        var genericSignal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);

        // Test first derivative at various points
        for (double t = -1.0; t <= 1.0; t += 0.2)
        {
            var float64Deriv = float64Signal.GetDerivative1Value(t);
            var genericDeriv = genericSignal.GetDerivative1Value(ScalarProcessor.ScalarFromNumber(t)).ScalarValue;
            Assert.That(Math.Abs(float64Deriv - genericDeriv), Is.LessThan(Tolerance),
                $"Derivative mismatch at t={t}: Float64={float64Deriv}, Generic={genericDeriv}");
        }

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteSymmetricTriangleSignal_Derivative2()
    {
        var float64Signal = Float64ScalarTriangleSignal.FiniteSymmetric;
        var genericSignal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);

        // Second derivative should be 0 everywhere (piecewise linear)
        for (double t = -1.0; t <= 1.0; t += 0.2)
        {
            var float64Deriv = float64Signal.GetDerivative2Value(t);
            var genericDeriv = genericSignal.GetDerivative2Value(ScalarProcessor.ScalarFromNumber(t)).ScalarValue;
            Assert.That(Math.Abs(float64Deriv - genericDeriv), Is.LessThan(Tolerance),
                $"Second derivative mismatch at t={t}: Float64={float64Deriv}, Generic={genericDeriv}");
            Assert.That(float64Deriv, Is.EqualTo(0.0).Within(Tolerance));
        }

        Debug.Assert(true);
    }

    [Test]
    public void TestPeriodicSymmetricTriangleSignal()
    {
        var float64Signal = Float64ScalarTriangleSignal.PeriodicSymmetric;
        var genericSignal = ScalarTriangleSignal<double>.PeriodicSymmetric(ScalarProcessor);

        for (double t = -1.0; t <= 1.0; t += 0.2)
        {
            var float64Value = float64Signal.GetValue(t);
            var genericValue = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t)).ScalarValue;
            Assert.That(Math.Abs(float64Value - genericValue), Is.LessThan(Tolerance),
                $"Mismatch at t={t}: Float64={float64Value}, Generic={genericValue}");
        }

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteToPeriodicConversion()
    {
        var finiteSignal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);
        var periodicSignal = finiteSignal.ToPeriodicSignal() as ScalarTriangleSignal<double>;

        Assert.That(periodicSignal, Is.Not.Null);
        Assert.That(periodicSignal!.IsPeriodic, Is.True);

        Debug.Assert(true);
    }

    [Test]
    public void TestPeriodicToFiniteConversion()
    {
        var periodicSignal = ScalarTriangleSignal<double>.PeriodicSymmetric(ScalarProcessor);
        var finiteSignal = periodicSignal.ToFiniteSignal() as ScalarTriangleSignal<double>;

        Assert.That(finiteSignal, Is.Not.Null);
        Assert.That(finiteSignal!.IsFinite, Is.True);

        Debug.Assert(true);
    }

    [Test]
    public void TestIsValid()
    {
        var finiteSignal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);
        var periodicSignal = ScalarTriangleSignal<double>.PeriodicSymmetric(ScalarProcessor);

        Assert.That(finiteSignal.IsValid(), Is.True);
        Assert.That(periodicSignal.IsValid(), Is.True);

        Debug.Assert(true);
    }

    [Test]
    public void TestVertexTimeProperty()
    {
        var signal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);
        Assert.That(signal.VertexTime.ScalarValue, Is.EqualTo(0.0).Within(Tolerance));
        Assert.That(signal.IsSymmetric, Is.True);

        var vertexTime = -0.5;
        var asymmetricSignal = ScalarTriangleSignal<double>.Finite(ScalarProcessor, ScalarProcessor.ScalarFromNumber(vertexTime));
        Assert.That(asymmetricSignal.VertexTime.ScalarValue, Is.EqualTo(vertexTime).Within(Tolerance));

        Debug.Assert(true);
    }

    [Test]
    public void TestOutOfRangeClamping()
    {
        var float64Signal = Float64ScalarTriangleSignal.FiniteSymmetric;
        var genericSignal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);

        // Test below minimum
        var t1 = -2.0;
        var float64Value1 = float64Signal.GetValue(t1);
        var genericValue1 = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t1)).ScalarValue;
        Assert.That(Math.Abs(float64Value1 - genericValue1), Is.LessThan(Tolerance),
            $"Clamping mismatch at t={t1}: Float64={float64Value1}, Generic={genericValue1}");

        // Test above maximum
        var t2 = 2.0;
        var float64Value2 = float64Signal.GetValue(t2);
        var genericValue2 = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t2)).ScalarValue;
        Assert.That(Math.Abs(float64Value2 - genericValue2), Is.LessThan(Tolerance),
            $"Clamping mismatch at t={t2}: Float64={float64Value2}, Generic={genericValue2}");

        Debug.Assert(true);
    }

    [Test]
    public void TestTimeRangeProperties()
    {
        var signal = ScalarTriangleSignal<double>.FiniteSymmetric(ScalarProcessor);

        Assert.That(signal.TimeRange.MinValue.ScalarValue, Is.EqualTo(-1.0));
        Assert.That(signal.TimeRange.MaxValue.ScalarValue, Is.EqualTo(1.0));

        Debug.Assert(true);
    }
}
