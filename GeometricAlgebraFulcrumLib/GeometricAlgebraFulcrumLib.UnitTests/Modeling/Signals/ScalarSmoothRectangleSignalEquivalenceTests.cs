...
[TestFixture]
public sealed class ScalarSmoothRectangleSignalEquivalenceTests
{
    private const double Tolerance = 1e-7; // Relaxed tolerance for complex exponential functions with many operations
    private static readonly ScalarProcessorOfFloat64 ScalarProcessor = ScalarProcessorOfFloat64.Instance;

    [Test]
    public void TestFiniteSmoothRectangleSignal_BoundaryValues()
    {
        var float64Signal = Float64ScalarSmoothRectangleSignal.FiniteInstance;
        var genericSignal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);
    ...
    [Test]
    public void TestFiniteSmoothRectangleSignal_NegativeRegion()
    {
        var float64Signal = Float64ScalarSmoothRectangleSignal.FiniteInstance;
        var genericSignal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);
    ...
    [Test]
    public void TestFiniteSmoothRectangleSignal_PositiveRegion()
    {
        var float64Signal = Float64ScalarSmoothRectangleSignal.FiniteInstance;
        var genericSignal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);
    ...
    [Test]
    public void TestFiniteSmoothRectangleSignal_SymmetryAroundZero()
    {
        var float64Signal = Float64ScalarSmoothRectangleSignal.FiniteInstance;
        var genericSignal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);

        // Test symmetry: f(-t) should equal f(t) for smooth rectangle
        for (double t = 0.1; t <= 0.9; t += 0.1)
        {
            ...
            Assert.That(Math.Abs(valuePlus - valueMinus), Is.LessThan(Tolerance),
                $"Symmetry broken at t=±{t}: f({t})={valuePlus}, f({-t})={valueMinus}");
        }
    ...
    [Test]
    public void TestFiniteSmoothRectangleSignal_Derivative1()
    {
        var float64Signal = Float64ScalarSmoothRectangleSignal.FiniteInstance;
        var genericSignal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);
    ...
    [Test]
    public void TestFiniteSmoothRectangleSignal_Derivative1AtBoundaries()
    {
        var float64Signal = Float64ScalarSmoothRectangleSignal.FiniteInstance;
        var genericSignal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);

        // Derivative should be 0 at boundaries
        var t1 = -1.0;
        ...
        // Derivative should be 0 at center
        var t3 = 0.0;
    ...
    [Test]
    public void TestFiniteSmoothRectangleSignal_Derivative2()
    {
        var float64Signal = Float64ScalarSmoothRectangleSignal.FiniteInstance;
        var genericSignal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);
    ...
    [Test]
    public void TestPeriodicSmoothRectangleSignal()
    {
        var float64Signal = Float64ScalarSmoothRectangleSignal.PeriodicInstance;
        var genericSignal = ScalarSmoothRectangleSignal<double>.Periodic(ScalarProcessor);
    ...
    {
        var finiteSignal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);
        var periodicSignal = finiteSignal.ToPeriodicSignal() as ScalarSmoothRectangleSignal<double>;
    ...
    {
        var periodicSignal = ScalarSmoothRectangleSignal<double>.Periodic(ScalarProcessor);
        var finiteSignal = periodicSignal.ToFiniteSignal() as ScalarSmoothRectangleSignal<double>;
    ...
    {
        var signal = ScalarSmoothRectangleSignal<double>.Finite(ScalarProcessor);
