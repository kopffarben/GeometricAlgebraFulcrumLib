...
[TestFixture]
public sealed class CosScalarSignalEquivalenceTests
{
    private const double Tolerance = 1e-12;
    private static readonly ScalarProcessorOfFloat64 ScalarProcessor = ScalarProcessorOfFloat64.Instance;

    [Test]
    public void TestFiniteCosSignal_BoundaryValues()
    {
        var float64Signal = Float64ScalarCosSignal.FiniteInstance;
        var genericSignal = CosScalarSignal<double>.Finite(ScalarProcessor);

        // Test at t = -π (left boundary, should be -1)
        var t1 = -Math.PI;
        ...
        Assert.That(float64Value1, Is.EqualTo(-1.0).Within(Tolerance)); // cos(-π) = -1
        ...
        Assert.That(float64Value2, Is.EqualTo(1.0).Within(Tolerance)); // cos(0) = 1

        // Test at t = π (right boundary, should be -1)
        var t3 = Math.PI;
        ...
        Assert.That(float64Value3, Is.EqualTo(-1.0).Within(Tolerance)); // cos(π) = -1

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteCosSignal_KeyPoints()
    {
        var float64Signal = Float64ScalarCosSignal.FiniteInstance;
        var genericSignal = CosScalarSignal<double>.Finite(ScalarProcessor);

        // Test at t = π/2 (should be ~0)
        var t1 = Math.PI / 2;
        var float64Value1 = float64Signal.GetValue(t1);
        var genericValue1 = genericSignal.GetValue(ScalarProcessor.ScalarFromNumber(t1)).ScalarValue;
        Assert.That(Math.Abs(float64Value1 - genericValue1), Is.LessThan(Tolerance),
            $"Mismatch at t={t1}: Float64={float64Value1}, Generic={genericValue1}");
        Assert.That(Math.Abs(float64Value1), Is.LessThan(Tolerance)); // cos(π/2) ≈ 0
        ...
        Assert.That(Math.Abs(float64Value2 - genericValue2), Is.LessThan(Tolerance),
            $"Mismatch at t={t2}: Float64={float64Value2}, Generic={genericValue2}");
        Assert.That(Math.Abs(float64Value2), Is.LessThan(Tolerance)); // cos(-π/2) ≈ 0

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteCosSignal_VariousPoints()
    {
        var float64Signal = Float64ScalarCosSignal.FiniteInstance;
        var genericSignal = CosScalarSignal<double>.Finite(ScalarProcessor);
    ...
    [Test]
    public void TestFiniteCosSignal_Derivative1()
    {
        var float64Signal = Float64ScalarCosSignal.FiniteInstance;
        var genericSignal = CosScalarSignal<double>.Finite(ScalarProcessor);

        // Test first derivative at various points (should be -sin(t))
        for (double t = -Math.PI; t <= Math.PI; t += Math.PI / 6)
        {
            ...
            Assert.That(Math.Abs(float64Deriv - expectedDeriv), Is.LessThan(Tolerance),
                $"Derivative should be -sin(t) at t={t}");
        }
    ...
    [Test]
    public void TestFiniteCosSignal_Derivative1AtKeyPoints()
    {
        var float64Signal = Float64ScalarCosSignal.FiniteInstance;
        var genericSignal = CosScalarSignal<double>.Finite(ScalarProcessor);

        // Derivative at t=0: -sin(0) = 0
        var t1 = 0.0;
        ...
        Assert.That(float64Deriv3, Is.EqualTo(1.0).Within(Tolerance));

        Debug.Assert(true);
    }

    [Test]
    public void TestFiniteCosSignal_Derivative2()
    {
        var float64Signal = Float64ScalarCosSignal.FiniteInstance;
        var genericSignal = CosScalarSignal<double>.Finite(ScalarProcessor);

        // Test second derivative at various points (should be -cos(t))
        for (double t = -Math.PI; t <= Math.PI; t += Math.PI / 6)
        {
            ...
            Assert.That(Math.Abs(float64Deriv - expectedDeriv), Is.LessThan(Tolerance),
                $"Second derivative should be -cos(t) at t={t}");
        }
    ...
    [Test]
    public void TestPeriodicCosSignal()
    {
        var float64Signal = Float64ScalarCosSignal.PeriodicInstance;
        var genericSignal = CosScalarSignal<double>.Periodic(ScalarProcessor);
    ...
    {
        var finiteSignal = CosScalarSignal<double>.Finite(ScalarProcessor);
        var periodicSignal = finiteSignal.ToPeriodicSignal() as CosScalarSignal<double>;
    ...
    {
        var periodicSignal = CosScalarSignal<double>.Periodic(ScalarProcessor);
        var finiteSignal = periodicSignal.ToFiniteSignal() as CosScalarSignal<double>;
    ...
    {
        var signal = CosScalarSignal<double>.Finite(ScalarProcessor);
    ...
    [Test]
    public void TestEvenFunctionProperty()
    {
        var genericSignal = CosScalarSignal<double>.Finite(ScalarProcessor);
        ...
        for (double t = 0.1; t <= Math.PI; t += Math.PI / 10)
        {
            ...
            Assert.That(Math.Abs(valuePlus - valueMinus), Is.LessThan(Tolerance),
                $"Even function property broken at t=±{t}: cos({t})={valuePlus}, cos({-t})={valueMinus}");
        }
