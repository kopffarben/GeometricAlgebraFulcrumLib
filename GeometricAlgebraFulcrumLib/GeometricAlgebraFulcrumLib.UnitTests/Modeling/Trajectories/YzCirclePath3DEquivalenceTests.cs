...
[TestFixture]
public sealed class YzCirclePath3DEquivalenceTests
{
    private const double Tolerance = 1e-12;

    [Test]
    public void TestYzCreate_Properties()
    {
        const double radius = 5.0;
        const int rotationCount = 1;

        var float64Path = new Float64YzCirclePath3D(radius, rotationCount);
        var genericPath = YzCirclePath3D<double>.Create(ScalarProcessorOfFloat64.Instance, radius, rotationCount);
    ...
    [Test]
    public void TestYzGetLength()
    {
        const double radius = 2.5;

        var float64Path = new Float64YzCirclePath3D(radius);
        var genericPath = YzCirclePath3D<double>.Create(ScalarProcessorOfFloat64.Instance, radius);
    ...
    [Test]
    public void TestYzGetValue_StartPoint()
    {
        ...
        Assert.That(genericPoint.X.ScalarValue, Is.EqualTo(0.0).Within(Tolerance), "X should be 0");
        Assert.That(genericPoint.Y.ScalarValue, Is.EqualTo(radius).Within(Tolerance), "Y should be radius");
        Assert.That(genericPoint.Z.ScalarValue, Is.EqualTo(0.0).Within(Tolerance), "Z should be 0");

        Debug.Assert(Math.Abs(genericPoint.Y.ScalarValue - radius) < Tolerance);
    }

    [Test]
    public void TestYzGetValue_QuarterCircle()
    {
        ...
        // At t=0.25 in YZ plane: (0, ~0, radius)
        Assert.That(genericPoint.X.ScalarValue, Is.EqualTo(float64Point.X.ScalarValue).Within(Tolerance));
        Assert.That(genericPoint.Y.ScalarValue, Is.EqualTo(float64Point.Y.ScalarValue).Within(Tolerance));
        Assert.That(genericPoint.Z.ScalarValue, Is.EqualTo(float64Point.Z.ScalarValue).Within(Tolerance));

        Assert.That(genericPoint.X.ScalarValue, Is.EqualTo(0.0).Within(Tolerance), "X should be 0");
        Assert.That(Math.Abs(genericPoint.Y.ScalarValue), Is.LessThan(Tolerance), "Y should be ~0");
    ...
    [Test]
    public void TestYzGetDerivative1Value()
    {
        const double radius = 5.0;
        const double t = 0.0;
    ...
    [Test]
    public void TestYzGetDerivative2Value()
    {
        const double radius = 3.0;
        const double t = 0.0;
    ...
    [Test]
    public void TestYzCenter_IsOrigin()
    {
        var genericPath = YzCirclePath3D<double>.Create(ScalarProcessorOfFloat64.Instance, 3.0);
    ...
    [Test]
    public void TestYzUnitNormal_PointsInX()
    {
        var genericPath = YzCirclePath3D<double>.Create(ScalarProcessorOfFloat64.Instance, 2.0);

        // YZ circle has normal in +X direction
        Assert.That(genericPath.UnitNormal.X.ScalarValue, Is.EqualTo(1.0).Within(Tolerance), "Normal X should be 1");
        Assert.That(genericPath.UnitNormal.Y.ScalarValue, Is.EqualTo(0.0).Within(Tolerance), "Normal Y should be 0");
    ...
    [Test]
    public void TestYzTimeToLength()
    {
    ...
    [Test]
    public void TestYzLengthToTime()
    {
