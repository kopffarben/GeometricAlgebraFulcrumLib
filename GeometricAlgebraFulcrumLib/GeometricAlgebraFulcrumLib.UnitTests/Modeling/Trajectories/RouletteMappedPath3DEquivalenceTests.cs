...
public sealed class RouletteMappedPath3DEquivalenceTests
{
    ...
    public void RouletteMappedPath3D_ShouldMatchFloat64()
    {
        ...
        foreach (var t in SampleParameters)
        {
            ...
            var floatLength = floatPath.TimeToLength(t).ScalarValue;
        ...
        var floatPathLength = floatPath.GetLength().ScalarValue;
        var lengths = new[] { 0.0, floatPathLength / 2.0, floatPathLength };
        foreach (var length in lengths)
        {
            var floatTime = floatPath.LengthToTime(length).ScalarValue;
