...
/// <summary>
/// Unit tests for CGA OpnsTangent Encoder equivalence - Milestone 1.2 continuation.
/// Tests ensure Float64 and Generic&lt;double&gt; encoders produce identical CGA blades.
///
/// OPNS Tangent encoding represents tangent spaces using Outer Product Null Space representation.
/// OpnsTangent is typically implemented as a wrapper around IpnsTangent with CGaDual() transformation.
///
/// Note: OpnsTangent is expected to have the same HyperPlane bug as IpnsTangent had.
/// Since IpnsTangent bugs were fixed, OpnsTangent should also work correctly.
/// </summary>
[TestFixture]
public class CGaOpnsTangentEncoderEquivalenceTests
{
    ...
    public void Point_2D_FromDoubles_ShouldProduceIdenticalBlades()
    {
        ...
        // Act
        var float64Blade = _float64Space4D.Encode.OpnsTangent.Point(x, y);
        var genericBlade = _genericSpace4D.Encode.OpnsTangent.Point(x, y);
        ...
        foreach (var pair in float64KVector.IdScalarPairs)
        {
            Assert.That(genericKVector.GetBasisBladeScalar(pair.Key).ScalarValue,
                ...
                $"OpnsTangent point term at {pair.Key} should match");
        }
    }
    ...
    public void Point_3D_FromDoubles_ShouldProduceIdenticalBlades()
    {
        ...
        // Act
        var float64Blade = _float64Space5D.Encode.OpnsTangent.Point(x, y, z);
        var genericBlade = _genericSpace5D.Encode.OpnsTangent.Point(x, y, z);
        ...
        foreach (var pair in float64KVector.IdScalarPairs)
        {
            Assert.That(genericKVector.GetBasisBladeScalar(pair.Key).ScalarValue,
                Is.EqualTo(pair.Value).Within(1e-14),
                $"OpnsTangent point 3D term at {pair.Key} should match");
        }
    }
    ...
    public void Line_2D_FromDistanceAndNormal_ShouldProduceIdenticalBlades()
    {
        ...
        // Act
        var float64Blade = _float64Space4D.Encode.OpnsTangent.Line(distance, normalX, normalY);
        var genericBlade = _genericSpace4D.Encode.OpnsTangent.Line(distance, normalX, normalY);
        ...
        foreach (var pair in float64KVector.IdScalarPairs)
        {
            Assert.That(genericKVector.GetBasisBladeScalar(pair.Key).ScalarValue,
                Is.EqualTo(pair.Value).Within(1e-14),
                $"OpnsTangent line term at {pair.Key} should match");
        }
    }
    ...
    public void Plane_3D_FromDistanceAndNormal_ShouldProduceIdenticalBlades()
    {
        ...
        // Act
        var float64Blade = _float64Space5D.Encode.OpnsTangent.Plane(distance, nx, ny, nz);
        var genericBlade = _genericSpace5D.Encode.OpnsTangent.Plane(distance, nx, ny, nz);
        ...
        foreach (var pair in float64KVector.IdScalarPairs)
        {
            Assert.That(genericKVector.GetBasisBladeScalar(pair.Key).ScalarValue,
                Is.EqualTo(pair.Value).Within(1e-14),
                $"OpnsTangent plane term at {pair.Key} should match");
        }
    }
    ...
    public void Point_AtOrigin_ShouldProduceIdenticalBlades()
    {
        ...
        foreach (var pair in float64KVector.IdScalarPairs)
        {
            Assert.That(genericKVector.GetBasisBladeScalar(pair.Key).ScalarValue,
                Is.EqualTo(pair.Value).Within(1e-14),
                $"OpnsTangent point at origin term at {pair.Key} should match");
        }
    }
    ...
    public void Plane_ThroughOrigin_ShouldProduceIdenticalBlades()
    {
        ...
        foreach (var pair in float64KVector.IdScalarPairs)
        {
            Assert.That(genericKVector.GetBasisBladeScalar(pair.Key).ScalarValue,
                Is.EqualTo(pair.Value).Within(1e-14),
                $"OpnsTangent plane through origin term at {pair.Key} should match");
        }
    }
